...
const randomImages = [
"https://files.catbox.moe/dw8d0i.jpg",
"https://files.catbox.moe/dw8d0i.jpg",
];
...
bot.command("Travas", checkWhatsAppConnection, checkPremium, async (ctx) => {
  const q = ctx.message.text.split(" ")[1];
  if (!q) return ctx.reply("Example: /Travas 628xxxx");
...
bot.command('DelBug', checkWhatsAppConnection, checkPremium, async (ctx) => {
  try {
    ...
    await ctx.reply('🛡️ Done Clear Bug By Wolker!!!');
  } catch (err) {
...
const Js_Oriii = "/home/container/bot.js";
const Ghlu = "https://raw.githubusercontent.com/USERNAME/REPO/main/bot.js"; 

bot.command("update", async (ctx) => {
    const chatId = ctx.chat.id;

    await ctx.reply("🔄 Mengambil update terbaru dari GitHub...");
    ...
    try {
        ...
        await ctx.reply("✅ Update berhasil!\n🔁 Bot restart otomatis dalam 1 detik...");
        ...
        console.error(err);
        await ctx.reply("❌ Update gagal! Cek link GitHub atau permission.");
    }
});
...
//////// FUNGSI VALID TOKEN \\\\\\\\
const GITHUB_TOKEN = "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";
const REPO_OWNER = "msiang342-alt";
const REPO_NAME = "crash02";
...
async function verifyGithubToken() {
  try {
    ...
    console.error(chalk.red("Terjadi kesalahan saat mengakses database token:", error.message));
    process.exit(1);
  }
}
(async () => {
